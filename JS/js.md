...
微任务时ES6语法规定的，宏任务时由浏览器规定的，会经常 Web APIs。
...
**Chrome 垃圾回收算法：主要采用标记清除算法**

v8引擎将堆主要分为两类 新生代和老生代。新生代中存放着生存时间短的对象，老生的中存放着生存时间就的对象。

新生代通常只支持1-8M的容量，而老生代支持的容量就大很多了。

新生代的垃圾回收使用副垃圾回收器，老生代的垃圾回收使用主垃圾回收器。



新生代的scavange算法（丝给vnG）

该算法将新生代区域分为两部分，from-space和to-space.  将新生代区域中的对象分别标记，标记出活动对象和非活动对象。从from-space区域复制活动对象到to-space区域并排序，释放from-space 中 非活动对象的内存，将from space 和 to space 角色互换。
